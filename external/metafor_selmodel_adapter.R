...
if (length(args) < 2) {
  stop("Usage: Rscript metafor_selmodel_adapter.R <input_csv> <output_json>")
}
...
fit <- tryCatch(
  {
    ...
    metafor::selmodel(res, type = "stepfun", steps = c(0.025, 1), alternative = "greater")
  },
  error = function(e) e
)
...
if (!all_finite(estimate, std_error, ci_low, ci_high, tau)) {
  write_json(args[2], "error", "metafor selection model returned non-finite estimates.")
  quit(save = "no", status = 0)
}
write_json(args[2], "ok", "metafor selection model", estimate, std_error, ci_low, ci_high, tau)
